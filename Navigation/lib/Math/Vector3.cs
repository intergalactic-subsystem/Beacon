...
namespace Navigation.lib.Math
{
    ...
    ///     Unit testing proved that NaN == NaN is not the same as Nan.Equals(NaN) - Altered .Equals method to be consistent
    ...
    /// </Changes>
    /// <remarks>
    ///     IComparable has been implemened on the Vector3 type however comparing two vectors has no meaning, we are comparing
    ///     the magnitude of two vectors for convinience.
        ...
        : IComparable, IComparable<Vector3>, IEquatable<Vector3>, IFormattable
    {
        ...
        /// </exception>
        public Vector3(IReadOnlyList<double> xyz)
        {
            if (xyz.Count == 3)
            {
                X = xyz[0];
        ...
        /// </summary>
        public double Magnitude => System.Math.Sqrt(SumComponentSqrs());
            ...
            get
            {
                switch (index)
                {
                    ...
                    case 2:
                        return Z;
                    default:
                        throw new ArgumentException(ThreeComponents, nameof(index));
                }
            }
        }
        ...
        {
            return
                (System.Math.Abs(v1.X - v2.X) < ZERO) &&
                (System.Math.Abs(v1.Y - v2.Y) < ZERO) &&
                (System.Math.Abs(v1.Z - v2.Z) < ZERO);
        }
        ...
        /// </implementation>
        public double MixedProduct(Vector3 otherV1, Vector3 otherV2)
        {
            return DotProduct(CrossProduct(this, otherV1), otherV2);
        }
        ...
        public static Vector3 Normalize(Vector3 v1)
        {
            ...
            if (double.IsInfinity(v1.Magnitude))
            {
                ...
                if (v1.IsNaN())
                    throw new Exception(NormalizeInf);
            }

            // Check that we are not attempting to normalize a vector of magnitude 0
            if (System.Math.Abs(v1.Magnitude) < ZERO)
                throw new Exception(Normalize0);

            // Check that we are not attempting to normalize a vector of NaN
            if (v1.IsNaN())
                throw new Exception(NormalizeNaN);
        ...
        public static Vector3 NormalizeOrDefault(Vector3 v1)
        {
            ...
            return v1.IsNaN() ? NaN : NormalizeOrNaN(v1);
        ...
        {
            if (!double.IsInfinity(v1.Magnitude)) return v1;

            var x = System.Math.Abs(v1.X) < ZERO
                ? 0
                : System.Math.Abs(v1.X - (-0)) < ZERO
                    ? -0
                    : double.IsPositiveInfinity(v1.X) ? 1 : double.IsNegativeInfinity(v1.X) ? -1 : double.NaN;
            var y = System.Math.Abs(v1.Y) < ZERO
                ? 0
                : System.Math.Abs(v1.Y - (-0)) < ZERO
                    ? -0
                    : double.IsPositiveInfinity(v1.Y) ? 1 : double.IsNegativeInfinity(v1.Y) ? -1 : double.NaN;
            var z = System.Math.Abs(v1.Z) < ZERO
                ? 0
                : System.Math.Abs(v1.Z - (-0)) < ZERO
                    ? -0
                    : double.IsPositiveInfinity(v1.Z) ? 1 : double.IsNegativeInfinity(v1.Z) ? -1 : double.NaN;
        ...
        /// <param name="control">
        ///     The interpolated point between the two vectors to retrieve (fraction between 0 and 1), or an
        ///     extrapolated point if allowed
        /// </param>
        /// <param name="allowExtrapolation">True if the control may represent a point not on the vertex between v1 and v2</param>
        /// <returns>
        ///     The value at an arbitrary distance (interpolation) between two vectors or an extrapolated point on the
        ///     extended virtex
        /// </returns>
        ...
        public static double Angle(Vector3 v1, Vector3 v2)
        {
            ...
            return
                System.Math.Acos(
                    System.Math.Min(1.0f, NormalizeOrDefault(v1).DotProduct(NormalizeOrDefault(v2))));
        }
        ...
        {
            var x = v1.X;
            var y = v1.Y*System.Math.Cos(rad) - v1.Z*System.Math.Sin(rad);
            var z = v1.Y*System.Math.Sin(rad) + v1.Z*System.Math.Cos(rad);
            return new Vector3(x, y, z);
        }
        ...
        {
            var x = v1.Z*System.Math.Sin(rad) + v1.X*System.Math.Cos(rad);
            var y = v1.Y;
            var z = v1.Z*System.Math.Cos(rad) - v1.X*System.Math.Sin(rad);
            return new Vector3(x, y, z);
        }
        ...
        {
            var x = v1.X*System.Math.Cos(rad) - v1.Y*System.Math.Sin(rad);
            var y = v1.X*System.Math.Sin(rad) + v1.Y*System.Math.Cos(rad);
            var z = v1.Z;
            return new Vector3(x, y, z);
        }
        ...
        public static Vector3 RotateX(Vector3 v1, double yOff, double zOff, double rad)
        {
            var x = v1.X;
            var y = v1.Y*System.Math.Cos(rad) - v1.Z*System.Math.Sin(rad) +
                    (yOff*(1 - System.Math.Cos(rad)) + zOff*System.Math.Sin(rad));
            var z = v1.Y*System.Math.Sin(rad) + v1.Z*System.Math.Cos(rad) +
                    (zOff*(1 - System.Math.Cos(rad)) - yOff*System.Math.Sin(rad));
            return new Vector3(x, y, z);
        }
        ...
        {
            var x = v1.Z*System.Math.Sin(rad) + v1.X*System.Math.Cos(rad) +
                    (xOff*(1 - System.Math.Cos(rad)) - zOff*System.Math.Sin(rad));
            var y = v1.Y;
            var z = v1.Z*System.Math.Cos(rad) - v1.X*System.Math.Sin(rad) +
                    (zOff*(1 - System.Math.Cos(rad)) + xOff*System.Math.Sin(rad));
            return new Vector3(x, y, z);
        }
        ...
        {
            var x = v1.X*System.Math.Cos(rad) - v1.Y*System.Math.Sin(rad) +
                    (xOff*(1 - System.Math.Cos(rad)) + yOff*System.Math.Sin(rad));
            var y = v1.X*System.Math.Sin(rad) + v1.Y*System.Math.Cos(rad) +
                    (yOff*(1 - System.Math.Cos(rad)) - xOff*System.Math.Sin(rad));
            var z = v1.Z;
            return new Vector3(x, y, z);
        }
        ...
        /// <acknowlagement>
        ///     Provided by Poggel, Steven Buraje in comments on CodeProject (
        ///     http://www.codeproject.com/Articles/17425/A-Vector-Type-for-C )
        /// </acknowlagement>
        public static Vector3 Projection(Vector3 v1, Vector3 v2)
        {
            return new Vector3(v2*(v1.DotProduct(v2)/System.Math.Pow(v2.Magnitude, 2)));
        }
        ...
        ///     A nonzero vector for which the vector rejection of this vector will be calculated. The
        ///     direction.
        /// </param>
        ...
        {
            this = Reflection(this, reflector);
            return this;
        }
        ...
        public static Vector3 Reflection(Vector3 v1, Vector3 v2)
        {
            ...
            if (System.Math.Abs(System.Math.Abs(v1.Angle(v2)) - System.Math.PI/2) < double.Epsilon)
                return -v1;

            var retval = new Vector3(2*v1.Projection(v2) - v1);
        ...
        /// <remarks>
        ///     If the fractional component of a is halfway between two integers, one of which is even and the other odd, then
        ///     the even number is returned.
        /// </remarks>
        public static Vector3 Round(Vector3 v1)
        {
            return new Vector3(System.Math.Round(v1.X), System.Math.Round(v1.Y), System.Math.Round(v1.Z));
        }
        ...
        {
            return new Vector3(System.Math.Round(v1.X, digits), System.Math.Round(v1.Y, digits),
                System.Math.Round(v1.Z, digits));
        }
        ...
        {
            return new Vector3(System.Math.Round(v1.X, mode), System.Math.Round(v1.Y, mode),
                System.Math.Round(v1.Z, mode));
        }
        ...
        {
            return new Vector3(System.Math.Round(v1.X, digits, mode), System.Math.Round(v1.Y, digits, mode),
                System.Math.Round(v1.Z, digits, mode));
        }
        ...
        {
            return new Vector3(System.Math.Round(X), System.Math.Round(Y), System.Math.Round(Z));
        }
        ...
        {
            return new Vector3(System.Math.Round(X, digits), System.Math.Round(Y, digits), System.Math.Round(Z, digits));
        }
        ...
        {
            return new Vector3(System.Math.Round(X, mode), System.Math.Round(Y, mode), System.Math.Round(Z, mode));
        }
        ...
        {
            return new Vector3(System.Math.Round(X, digits, mode), System.Math.Round(Y, digits, mode),
                System.Math.Round(Z, digits, mode));
        }
        ...
        public string ToVerbString()
        {
            ...
            if (IsUnitVector())
                ...
                output += PositionalVector;

            output += $"( x={X}, y={Y}, z={Z} )";
            output += OfMagnitude + Magnitude;
        ...
        /// </summary>
        /// <param name="format">
        ///     Formatting string: 'x','y','z' or '' followed by standard numeric format string characters valid
        ///     for a double precision floating point
        /// </param>
        ...
        /// </implementation>
        /// <remarks>
        ///     NaN and NaN components will be equal in this method but not in ==, see
        ...
        {
            if (other is Vector3)
                return Equals((Vector3) other, tolerance);
            return false;
        }
        ...
        {
            if (other is Vector3)
                return CompareTo((Vector3) other, tolerance);
        ...
        {
            return System.Math.Abs(v1.Magnitude - 1) < ZERO;
        }
        ...
        ///     Uses the <see cref="NormalizeOrDefault(Vector3)" /> function in the check to avoid code duplication and to include
        ///     spcial cased of inifite values.
        ///     Of the three flavours of AlmostEquals that have been implemented the absolute value tolerance is the only one
        ///     that makes senses when comparing to 0. If we later implement relative and/or ulp tolerance, this method would not
        ///     benefit from the overloads.
        /// </implementation>
        /// <remarks>
        ...
        /// <implementation>
        ///     Uses the <see>
        ///         <cref>IsPerpendicualr(Vector3, Vector3)</cref>
        ///     </see>
        ///     function in the check to avoid code duplication.
        /// </implementation>
        ...
        /// <implementation>
        ///     Uses the <see>
        ///         <cref>IsPerpendicualr(Vector3, Vector3, double)</cref>
        ///     </see>
        ///     function in the check to avoid code duplication.
        ///     Of the three flavours of AlmostEquals that have been implemented the absolute value tolerance is the only one
        ///     that makes senses when comparing to 0. If we later implement relative and/or ulp tolerance, this method would not
        ...
        /// </summary>
        private const string Normalize0 = "Cannot normalize a vector when it's magnitude is zero";

        /// <summary>
        ///     Exception message descriptive text,
        ///     used when an attempt is made to normalize a vector with an infinite magnitude.
        /// </summary>
        private const string NormalizeInf =
            "Cannot normalize a vector when it's magnitude is infinite except under special conditions";

        /// <summary>
        ///     Exception message descriptive text,
        ///     used for a failure for an array argument to have three components when three are needed .
        /// </summary>
        private const string ThreeComponents = "Array must contain exactly three components , (x,y,z)";
        ...
        /// </summary>
        private const string NonVectorComparison = "Cannot compare a Vector3 to a non-Vector3";
        ...
        /// </summary>
        private const string ArgumentType = "The argument provided is a type of ";
        ...
        /// </summary>
        private const string ArgumentLength = "The argument provided has a length of ";

        /// <summary>
        ///     Exception message descriptive text
        ///     Used when attempting to set a vectors magnitude to a negative value
        /// </summary>
        private const string NegativeMagnitude =
        ...
        private const string UnitVector = "Unit vector composing of ";

        private const string PositionalVector = "Positional vector composing of  ";

        private const string OfMagnitude = " of magnitude ";
