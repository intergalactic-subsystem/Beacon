...
namespace Navigation.lib.Math
{
    public static class DoubleExtension
    {
        ...
        /// <remarks>
        ///     Use this tolerant equality method if comparing against zero. The tolerance should be a small multiple of
        ...
        ///     If you are comparing against a non-zero number then relative epsilons or ULPs based comparisons are probably what
        ///     you want.
            ...
            double maxRelativeError)
        {
            ...
            var relativeError = absB > absA ? System.Math.Abs((a - b)/b) : System.Math.Abs((a - b)/a);

            return relativeError <= maxRelativeError;
        }

        /// <summary>
        ///     Comparator within a numer of units in the last place that we are tolerant of (the numerb of "floats" difference
        ///     that is acceptable)
        /// </summary>
        /// <param name="a">The double to compare to</param>
        /// <param name="b">The double to compare with</param>
            ...
            long maxUlps)
        {
            ...
            var longA = BitConverter.DoubleToInt64Bits(a);
            longA = longA < 0 ? (long) (0x8000000000000000 - (ulong) longA) : longA;

            var longB = BitConverter.DoubleToInt64Bits(b);
            longB = longB < 0 ? (long) (0x8000000000000000 - (ulong) longB) : longB;

            var diff = System.Math.Abs(longA - longB);
            return diff <= maxUlps;
        }
    }
}